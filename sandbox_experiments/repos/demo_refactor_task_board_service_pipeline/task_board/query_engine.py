...
def task_matches_filters(
    ...
    *,
    ...
    owner_and_status_ok = owner_ok and status_ok
    return owner_and_status_ok and archived_ok
...
def task_matches_export_filters(
    task: dict[str, Any],
    *,
    owner: str,
    status: str,
    ...
    """Return whether the task should appear in export output."""
    owner_ok = owner_matches(task, owner)
    status_ok = status_matches(task, status)
    archived_ok = archived_matches(task, include_archived)
    return owner_ok and status_ok and archived_ok


def collect_export_tasks(
    tasks: list[dict[str, Any]],
    *,
    owner: str,
    status: str,
    ...
    """Collect tasks that match the export filters."""
    matches: list[dict[str, Any]] = []
    for task in tasks:
        if task_matches_export_filters(
            task,
            owner=owner,
            status=status,
            include_archived=include_archived,
...
def render_task_list(
    ...
    *,
    ...
    renderable_tasks = build_renderable_tasks(sorted_tasks)
...
def build_export_header() -> str:
    """Render the stable CSV header used by export."""
    return "id,title,owner,status,archived"


def render_export_row(task: dict[str, Any]) -> str:
    """Render a single CSV export row."""
    archived_text = "true" if get_task_archived(task) else "false"
    return (
        f"{get_task_id(task)},{get_task_title(task)},"
        f"{get_task_owner(task)},{get_task_status(task)},{archived_text}"
    )


def render_export_list(
    tasks: list[dict[str, Any]],
    *,
    owner: str,
    status: str,
    ...
    """Filter, sort, and render the export output."""
    original_tasks = ensure_list(tasks)
    matching_tasks = collect_export_tasks(
        original_tasks,
        owner=owner,
        status=status,
        include_archived=include_archived,
    ...
    rows = [build_export_header()]
    for task in sorted_tasks:
        rows.append(render_export_row(task))
    return "\n".join(rows)


def build_owner_digest_lines(tasks: list[dict[str, Any]]) -> list[str]:
    """Build a stable owner summary for matching tasks."""
    counts: dict[str, int] = {}
    for task in tasks:
    ...
    for owner in sorted(counts):
        lines.append(f"{owner}: {counts[owner]} task(s)")
    return lines
...
def render_owner_digest(
    ...
    *,
    ...
    """Filter, sort, and summarize matching tasks by owner."""
    original_tasks = ensure_list(tasks)
    prepared_tasks = passthrough_tasks(original_tasks)
    _debug_snapshot = build_filter_debug_snapshot(
        prepared_tasks,
    ...
    if not renderable_tasks:
        return "No owners matched."

    lines = build_owner_digest_lines(renderable_tasks)
    return "\n".join(lines)


def build_priority_digest_lines(tasks: list[dict[str, Any]]) -> list[str]:
    """Build a stable priority summary for matching tasks."""
    counts: dict[str, int] = {}
    for task in tasks:
        label = get_priority_label(task)
        counts[label] = counts.get(label, 0) + 1

    lines: list[str] = []
    for label in sorted(counts):
        lines.append(f"{label}: {counts[label]} task(s)")
    return lines


def render_priority_digest(
    tasks: list[dict[str, Any]],
    *,
    owner: str,
    status: str,
    ...
    """Filter, sort, and summarize matching tasks by priority."""
    original_tasks = ensure_list(tasks)
    prepared_tasks = passthrough_tasks(original_tasks)
    _debug_snapshot = build_filter_debug_snapshot(
        prepared_tasks,
    ...
    if not renderable_tasks:
        return "No priorities matched."

    lines = build_priority_digest_lines(renderable_tasks)
    return "\n".join(lines)
