...
- comparison summary 已新增 task-level 差异展开，当前除聚合 deltas 外，还会输出 task_deltas。
- 每条 task delta 会记录 outcome、passed、step_count、tool_call_count、verify_count、reflect_count、reflect_trigger_reason、failure_taxonomy 以及 failing_checks / diagnostic_labels 的差异。
- comparison Markdown 已新增 Task Delta 小节，便于直接定位是哪条任务把两套策略拉开。
- 当前 pytest -q 已通过，结果为 18 passed。