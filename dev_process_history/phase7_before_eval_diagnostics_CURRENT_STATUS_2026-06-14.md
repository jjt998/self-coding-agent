...
- 继续推进 `Phase 7`，下一步开始补 diagnostics 流程与更细的评测指标。

## 下一步明确动作

- 为 eval summary 增加更明确的 diagnostics 流程，区分成功但有风险提示和真正失败。
- 评估是否需要把 memory 标签、stop reason 和验证结果整理成更系统的 failure taxonomy。
- 评估是否需要让 eval task schema 支持任务级配置覆盖以外的断言或期望字段。
...
5. 从 eval diagnostics、failure taxonomy 或 task-level expectations 开始继续。
