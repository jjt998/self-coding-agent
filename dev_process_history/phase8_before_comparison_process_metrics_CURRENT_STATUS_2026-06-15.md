...
- 继续推进 `Phase 8`，下一步优先增强 comparison summary，让策略对比结果能直接展示 context / memory / reflect 策略名与关键 delta。

## 下一步明确动作

- 评估 comparison summary 是否需要补充 context / memory / reflect 策略名、代表性 trace 链接或 failure taxonomy delta。
- 评估是否需要把 reflect 次数、触发原因分布也纳入 eval / comparison 聚合指标。
...
5. 优先查看 `src/eval_runner.py`、`src/loop.py`、`tests/test_eval.py` 与 `tests/test_loop.py`，从 comparison summary 指标增强继续推进。
