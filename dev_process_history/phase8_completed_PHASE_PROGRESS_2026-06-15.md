...
- Phase 8: comparison summary 已继续增强解释力。
- 本次新增 failure_taxonomy_counts_delta、failure_taxonomy_tag_counts_delta 和 verification_failure_counts_delta。
- comparison Markdown 现在会同步展示 failure taxonomy delta、verification checks delta 和 reflect 原因 delta。
- 新增 tests/test_eval_phase8_comparison.py，专门覆盖 comparison failure delta 字段与 Markdown 输出。
- 验收结果：pytest -q 通过，当前为 18 passed。
## 2026-06-15 再次补充更新

- Phase 8：comparison summary 已从聚合 delta 扩展到 task-level delta。
- 本次新增 task_deltas，并在 comparison Markdown 中加入 Task Delta 小节。
- task-level delta 当前覆盖 outcome、passed、steps、tool calls、verify、reflect、reflect reason、failure taxonomy，以及 failing checks / diagnostic labels 差异。
- 新增的回归测试继续放在 tests/test_eval_phase8_comparison.py。
- 验收结果：pytest -q 通过，当前为 18 passed。