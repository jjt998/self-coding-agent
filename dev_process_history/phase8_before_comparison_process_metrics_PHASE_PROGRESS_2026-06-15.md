...
- 剩余：
  - 评估 comparison summary 是否需要补充 context / memory / reflect 策略名、代表性 trace 分析或 failure taxonomy delta
  - 评估是否把 reflect 次数、触发原因分布纳入 eval / comparison 聚合指标
- 验收：
  - 同一任务集上至少两种策略可比较，并输出 delta。
- 备注：
  - 第一批实验继续保持窄范围、强控制，优先验证“能不能稳定比较”，再扩展策略维度。
  - 当前已具备 context、memory 和 reflect 三个维度的最小可比策略，下一步优先增强 comparison summary 的可读性和解释力。
