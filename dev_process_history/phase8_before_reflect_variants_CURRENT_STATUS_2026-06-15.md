...
- 继续推进 `Phase 8`，下一步优先补 `reflect trigger variants`，让策略对比不只停留在 context / memory 维度。

## 下一步明确动作

- 增加第一版 reflect trigger variants，例如 `verify_failure_only_reflect` 与 `low_progress_plus_verify_reflect`。
- 评估 comparison summary 是否需要补充 context strategy 名称、代表性 trace 链接或 failure taxonomy delta。
...
5. 优先查看 `src/context.py`、`src/loop.py`、`src/eval_runner.py` 与 `tests/test_cli.py`，从 reflect 策略开关继续推进。
