...
- 继续推进 `Phase 8`，下一步从 context strategy variants 与 reflect trigger variants 两条线里选一条继续补齐对比维度。

## 下一步明确动作

- 增加第一版 context strategy variants，例如 `naive_recent_context` 与 `file_recall_context` 的可切换入口。
- 增加第一版 reflect trigger variants，例如 `verify_failure_only_reflect` 与 `low_progress_plus_verify_reflect`。
- 评估 comparison summary 是否需要补充代表性 trace 链接或 failure taxonomy delta。
...
5. 优先查看 `src/eval_runner.py`、`src/memory.py`、`src/cli.py` 与 `tests/test_eval.py`，从 context / reflect 策略开关继续推进。
