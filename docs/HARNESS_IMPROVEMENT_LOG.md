...
- Action（动作）：这轮我没有押注单点修补，而是把问题拆成四条治理链路一起处理。第一条是“收口机制治理”：把 verify 收敛为末尾单次裁判，loop 退出改为由 harness 根据 `tool_calls` 是否为空、连续空调用、`max_steps` 和错误状态统一判断，避免模型在过程里把 verify 当作提示器；同时在 prompt 里明确 `bug_fix` 收口规则：当目标 diff 已命中、核心验证命令已符合预期，就优先减少读取并让 `tool_calls` 收空。第二条是“运行时记忆治理”：把旧的 `donelist` 升级为结构化 `working_memory`，最终收紧为 `confirmed_facts / invalidated_beliefs / completed_actions / next_risks` 四类，只保留更有收口价值的状态；被当前轮代码、命令输出或 diff 直接否定的旧怀疑必须进入 `invalidated_beliefs`，而不是继续维护一个会驱动发散探索的 `open_questions`。第三条是“文件上下文污染治理”：对编辑过的文件做整文件级失效，成功 `apply_patch` / `replace_lines` 后旧缓存统一标记为 `stale`；重读路径固定为 `read_file_structure_summary -> read_file_range`，并新增 `reread_fresh_ranges` / `safe_to_rely_ranges`，显式告诉模型“这个文件虽然历史上 stale 过，但这些范围已经按当前源码重读，可以直接信”。第四条是“证据优先级治理”：把 `git_diff` 改为完整 diff 注入而不是摘要，失败 `run_command` 的 stderr 摘要和 Python traceback 结构化信息接进 `recent_tool_results` / `failed_tools`，并在 prompt 里明确写死：任务描述描述的是待修复现象，不保证与当前轮已修改后的文件内容一致；当任务描述与当前代码冲突时，优先相信 fresh 代码、命令输出、diff 和最近工具结果，而不是继续把初始描述当成当前事实。另外还补了一条更硬的规则：当关键目标函数已经是 fresh 且已直接读到实现时，不要因为任务描述冲突就立刻扩展读取外围 helper，应先把冲突写入 `invalidated_beliefs` 或直接在 rationale 中说明，并优先运行核心命令校验当前代码行为。
