...
- 第二轮模型的 `context_snapshot.recent_facts.recent_tool_results` 是否包含目标文件 `path` 和短源码片段。
...
- 结构化工作记忆是否能稳定承接累计已完成事项，并通过 `context_snapshot.working_memory` 影响下一轮 plan。
...
- Action（动作）：这轮我没有押注单点修补，而是把问题拆成四条治理链路一起处理。第一条是“收口机制治理”：把 verify 收敛为末尾单次裁判，loop 退出改为由 harness 根据 `tool_calls` 是否为空、连续空调用、`max_steps` 和错误状态统一判断，避免模型在过程里把 verify 当作提示器；同时在 prompt 里明确 `bug_fix` 收口规则：当目标 diff 已命中、核心验证命令已符合预期，就优先减少读取并让 `tool_calls` 收空。第二条是“运行时记忆治理”：把旧的 `donelist` 升级为结构化 `working_memory`，最终进一步收紧为 `confirmed_facts / invalidated_beliefs / completed_actions / next_risks` 四类，只保留更有收口价值的状态；我明确去掉了 `open_questions`，因为真实 run 证明这个字段会天然鼓励模型继续扩展阅读、不断补“待确认问题”，从而把保守性放大成发散性。被当前轮代码、命令输出或 diff 直接否定的旧怀疑，统一进入 `invalidated_beliefs`。第三条是“文件上下文污染治理”：对编辑过的文件做整文件级失效，成功 `apply_patch` / `replace_lines` 后旧缓存统一标记为 `stale`；重读路径固定为 `read_file_structure_summary -> read_file_range`，并新增 `reread_fresh_ranges` / `safe_to_rely_ranges`，显式告诉模型“这个文件虽然历史上 stale 过，但这些范围已经按当前源码重读，可以直接信”。第四条是“证据优先级治理”：把 `git_diff` 改为完整 diff 注入而不是摘要，失败 `run_command` 的 stderr 摘要和 Python traceback 结构化信息接进 `recent_tool_results` / `failed_tools`，并在 prompt 里明确写死：任务描述描述的是待修复现象，不保证与当前轮已修改后的文件内容一致；当任务描述与当前代码冲突时，优先相信 fresh 代码、命令输出、diff 和最近工具结果，而不是继续把初始描述当成当前事实。另外，我只把“当前是第几轮”作为轻量字段注入模型，不暴露 `max_steps` 和剩余预算，让模型能区分“第 2 轮的正常探索”和“第 12 轮还在重复确认”的语义阶段，却不会因为知道预算而产生投机式收口。
- Result（结果）：这组治理不是一次性把所有问题清零，但真实 run 已经显示出非常明确的正反馈，而且在后续继续收紧 `working_memory` 与补入当前轮次后，效果还进一步增强。以 `demo_buggy_task_board_dual_fix` 为例，早期 run 会把 `archived_matches` 也误改掉，或者虽然功能基本修好但一路拖到 `max_steps_reached`；而在后续 run `run-20260622-181449-00f7e1c0` 中，模型最终只修改了真正需要的两处核心逻辑，`verification_diff_snapshot` 也保持干净，`run_finished.stop_reason.details.solve_loop_exit_reason` 收敛为 `no_planned_tool_calls`，说明 agent 开始能主动结束而不是被预算硬截停。再往后，在去掉 `open_questions`、只保留四字段 `working_memory`，并把 `current_iteration` 注入模型之后，真实 bug_fix run 又出现了更进一步的改善：正确率更高、收敛轮数明显下降，而且行为更稳，不再频繁出现“已经修对但还要围绕待确认问题反复扩展读取”的长尾。换句话说，这次不是只改善了 trace 可解释性，而是实打实提高了任务完成质量和收敛效率。与此同时，`previous_reflect` 中已经能稳定看到 `stale_file_paths=[]`、`reread_fresh_ranges=[...]` 与 `safe_to_rely_ranges`，表明“历史 stale”和“当前 fresh”在模型侧终于被拆开表达。相关回归测试也已经覆盖并通过：`tests/test_model.py tests/test_loop.py tests/test_mvp_acceptance_docs.py -q` 为 `55 passed`，并额外验证了当前轮次注入与四字段 `working_memory` 的链路一致性。
- Benefit（收益）：这次改进最大的收获，不是“又加了几个 prompt 规则”，而是把我对 agent 失败根因的认识推进了一层。我一开始以为主要问题是模型不够聪明，后来发现更核心的是 harness 没把“证据的可信度层级”和“运行时记忆的新鲜度”表达清楚：任务描述、旧片段、fresh 代码、命令输出、diff、验证结果如果混在一起，模型就会非常保守，表现为不敢收口、重复读文件和反复兜圈。更关键的是，我后面又验证了一个很有价值的经验：**并不是结构化字段越多越好**。`open_questions` 看起来合理，但在真实 bug_fix 任务里，它会把模型的谨慎放大成发散，逼着模型不断为“待确认问题”补阅读、补命令、补外围函数检查；反而去掉它，只保留事实、已证伪判断、已完成动作和风险，再给模型一个“当前是第几轮”的轻量阶段信号，会让它更容易判断“现在应该继续查，还是应该结束”。这次效果非常明显：正确率和收敛轮数都出现了肉眼可见的改善，而且行为更稳，说明这不是偶然跑中，而是 harness 语义设计真正贴近了模型的决策方式。对我自己来说，这个过程最大的工程收获是：做 agent harness 不能只盯模型回答质量，而要把“状态语义、证据优先级、上下文污染、终止条件和 trace 可解释性”当成同一件事来设计；否则问题会在真实 run 中以“模型变笨了”的形式出现，但根因其实在 harness。
...
- 是否固化：已固化为当前 Phase 9.x 的一组核心 harness 能力，包括：末尾单次 verify、`tool_calls` 收口、四字段 `working_memory`、文件编辑后的 `stale -> reread -> fresh` 链路、`reread_fresh_ranges`、当前轮次注入、以及围绕任务描述/代码/命令/diff 冲突时的证据优先级约束。特别是“去掉 `open_questions` + 只注入当前轮次”这一步，已经在真实任务里体现出明确有效性，我会把它视为这轮最关键的稳定性增强之一，而不是可有可无的小调参。后续仍会继续观察更大仓库和更复杂 bugfix 任务，但当前证据已经足够支持把这套设计作为有效固化方案保留下来。
