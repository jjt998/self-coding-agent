...
- `summary.json` 包含 `failure_taxonomy_counts`、`failure_taxonomy_tag_counts`、`observe_trigger_reason_counts`。
...
- delta 中包含 `failure_taxonomy_counts_delta`、`failure_taxonomy_tag_counts_delta`、`verification_failure_counts_delta`、`observe_trigger_reason_counts_delta`。
...
- 增强模型重规划质量评估，而不是只校验是否消费了上一轮 `observe_content`。
